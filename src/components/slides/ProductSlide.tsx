        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          {features.slice(0, 3).map((f, i) => (
            <motion.div
              key={i}
              variants={item}
              className="slide-card rounded-xl p-6 slide-card-hover transition-all duration-300 group relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-24 h-24 opacity-0 group-hover:opacity-100 transition-opacity" style={{
                background: 'radial-gradient(circle at top right, hsl(260 60% 55% / 0.1), transparent 70%)',
              }} />
              <div className="relative flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-lg icon-container flex items-center justify-center">
                  <f.icon className="w-5 h-5 text-primary" />
                </div>
                <span className="text-[10px] font-mono font-medium text-accent/80 bg-accent/[0.08] px-2 py-0.5 rounded border border-accent/10">{f.badge}</span>
              </div>
              <h3 className="relative text-base font-semibold font-display mb-2">{f.title}</h3>
        ...
        <div className="grid grid-cols-2 gap-4 flex-1">
          {features.slice(3).map((f, i) => (
            <motion.div
              ...
              className="slide-card rounded-xl p-6 slide-card-hover transition-all duration-300 group relative overflow-hidden"
            >
              <div className="absolute bottom-0 left-0 w-32 h-32 opacity-0 group-hover:opacity-100 transition-opacity" style={{
                background: 'radial-gradient(circle at bottom left, hsl(185 80% 50% / 0.08), transparent 70%)',
              }} />
              <div className="relative flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-lg icon-container flex items-center justify-center">
                  <f.icon className="w-5 h-5 text-primary" />
